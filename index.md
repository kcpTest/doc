...
주문요청 페이지는 주문자로부터 결제요청 데이터를 입력 받고, 가맹점이 설정한 데이터를 체크 후 결제를 요청하는 페이지입니다. NHN KCP가 제공하는 샘플소스 상에서는 \[order\_mobile\] 페이지에 해당하며, 실제 페이지 작성시에는 샘플소스와 매뉴얼을 참고하시어 가맹점에 맞게 수정, 적용하시기 바랍니다.

※ 주문요청 페이지에서 결제창을 띄우는 방식은 **페이지전환 방식**이며, 반드시 샘플소스를 참고하시어 **페이지전환 방식**으로 결제창을 호출해야 합니다. **페이지전환 방식**이 아닌 iframe, 팝업 방식 등 업체 자체적으로 수정하여 호출을 하게 될 경우 디바이스 환경 및 브라우저에 따라 결제 진행이 원활하지 않을 수 있습니다. 이점 반드시 유의하시기 바랍니다.
